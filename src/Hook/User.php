...
class User {
    ...
    public static function user_authenticate( $result, $username ){
        do_action( 'log', 'info', 'Login attempt for user: {username}', array( 'username' => $username, 'source' => 'wordpress.user', 'sensitive' => true ) );
        return $result;
    }
  
    public static function user_login_failed( $username ){
        do_action( 'log', 'warning', 'Login failed for user: {username}',  array( 'username' => $username, 'source' => 'wordpress.user' ) );
    }
    
    public static function user_created( $user_id ){
        do_action( 'log', 'info', 'Created user ID: {user_id}', array( 'user_id' => $user_id, 'source' => 'wordpress.user' ) );
    }    
  
    public static function user_updated( $user_id ){
        do_action( 'log', 'info', 'Updated user ID: {user_id}', array( 'user_id' => $user_id, 'source' => 'wordpress.user' ) );
    }   
    
    public static function user_deleted( $user_id ){
        do_action( 'log', 'info', 'Deleted user ID: {user_id}', array( 'user_id' => $user_id, 'source' => 'wordpress.user' ) );
    }
